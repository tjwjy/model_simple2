...
##____simulate single center
    def simulate_home_repeat1(self):
        ...
        while (simulate_time < self.simulate_time):
            ...
            if (t_now):
                # pass
                temp_Model = move_rules2.HomeOrWork_Model_repeat1(self.args_model, self.Envir, self.home_loc, self.work_loc,
                                                         self.home_locationList)
                temp_Model.set_t_constraint(t_now=t_now, t_end=self.rest_time[0], args_t=self.args_t)
                temp_Model.set_space_constrain(self.args_step)
                ...
                ##____simulate single center
    def simulate_home_repeat1_exp(self):
        #change the weight
        print([self.home_loc.x, self.home_loc.y])
        print([self.work_loc.x, self.work_loc.y])
        simulate_time = 0
        ...
        while (simulate_time < self.simulate_time):
            ...
            if (t_now):
                # pass
                temp_Model = move_rules2.HomeOrWork_Model_repeat1_exp(self.args_model, self.Envir,
                                                                  self.home_loc, self.work_loc,
                                                                  self.home_locationList)
                temp_Model.set_t_constraint(t_now=t_now, t_end=self.rest_time[0], args_t=self.args_t)
                temp_Model.set_space_constrain(self.args_step)
                self.home_locationList, tempRoute, S = temp_Model.get_route(self.home_loc, S)
    ...
    def simulate_home_repeat1_lognormal(self):
        # change the weight
        print([self.home_loc.x, self.home_loc.y])
        print([self.work_loc.x, self.work_loc.y])
        simulate_time = 0
        ...
        while (simulate_time < self.simulate_time):
            ...
            if (t_now):
                # pass
                temp_Model = move_rules2.HomeOrWork_Model_repeat1_lognormal(self.args_model, self.Envir,
                                                                      self.home_loc, self.work_loc,
                                                                      self.home_locationList)
                temp_Model.set_t_constraint(t_now=t_now, t_end=self.rest_time[0], args_t=self.args_t)
                temp_Model.set_space_constrain(self.args_step)
                ...
                simulate_time += 1
##____simulate single center
    def simulate_home_repeat2(self):
        print ([self.home_loc.x, self.home_loc.y])
        print ([self.work_loc.x, self.work_loc.y])
        simulate_time = 0
        S=0
        while (simulate_time < self.simulate_time):
            ...
            if (t_now):
                # pass
                temp_Model = move_rules2.HomeOrWork_Model_repeat2(self.args_model, self.Envir, self.home_loc, self.work_loc,
                                                         self.home_locationList)
                temp_Model.set_t_constraint(t_now=t_now, t_end=self.rest_time[0], args_t=self.args_t)
                temp_Model.set_space_constrain(self.args_step)
                self.home_locationList, tempRoute,S = temp_Model.get_route(self.home_loc,S)
                ...
                simulate_time += 1
                # do something around work
    def simulate_home_repeatnew2(self):
        print ([self.home_loc.x, self.home_loc.y])
        print ([self.work_loc.x, self.work_loc.y])
        simulate_time = 0
        S=0
        while (simulate_time < self.simulate_time):
            ...
            if (t_now):
                # pass
                temp_Model = move_rules2.HomeOrWork_Model_repeatnew2(self.args_model, self.Envir, self.home_loc, self.work_loc,
                                                         self.home_locationList)
                temp_Model.set_t_constraint(t_now=t_now, t_end=self.rest_time[0], args_t=self.args_t)
                temp_Model.set_space_constrain(self.args_step)
                self.home_locationList, tempRoute,S = temp_Model.get_route(self.home_loc,S)
                ...
                simulate_time += 1
                # do something around work
    def simulate_home_repeatnew1(self):
        print ([self.home_loc.x, self.home_loc.y])
        print ([self.work_loc.x, self.work_loc.y])
        simulate_time = 0
        S=0
        while (simulate_time < self.simulate_time):
            ...
            if (t_now):
                # pass
                temp_Model = move_rules2.HomeOrWork_Model_repeatnew1(self.args_model, self.Envir, self.home_loc, self.work_loc,
                                                         self.home_locationList)
                temp_Model.set_t_constraint(t_now=t_now, t_end=self.rest_time[0], args_t=self.args_t)
                temp_Model.set_space_constrain(self.args_step)
                self.home_locationList, tempRoute,S = temp_Model.get_route(self.home_loc,S)
...
########################################################################################################################
##-dual-centric
    def simulate_dualcentric_repeat(self):
        print ([self.home_loc.x, self.home_loc.y])
        print ([self.work_loc.x, self.work_loc.y])
        simulate_time = 0
        S=0
        while (simulate_time < self.simulate_time):
            ...
            if (t_now < self.work_time[0] and t_now >= self.rest_time[1]):
                # pass
                temp_Model = move_rules2.Commute_Model_repeat1(self.args_model, environment=self.Envir,
            ...
            if (t_now > self.work_time[1] and t_now < self.rest_time[0]):
                ...
                while (t_now < self.rest_time[0]):
                    ...
                    if (temp < 0.9):
                        break
                    temp_Model = move_rules2.Commute_Model_repeat1(self.args_model, self.Envir, self.home_loc, self.work_loc,
                                                          self.commute_LocationList)
            ...
            if (t_now > self.rest_time[0] or t_now < self.rest_time[1]):
                ...
                simulate_time += 1
                # print (simulate_time)
    def simulate_dualcentric_t(self):
        simulate_time = 0
        S=0
        tList=[]
        while (simulate_time < self.simulate_time):
            ...
            if (t_now > self.work_time[0] and t_now < self.work_time[1]):
                ...
                for item in ts:
                    t_now = (t_now + item)
                    tList.append(t_now % 24)
                    if(t_now> self.work_time[1]):
                        break
                # do something around work
            if (t_now > self.work_time[1] and t_now < self.rest_time[0]):
                # pass
    ...
    def simulate_dualcentric24(self,p):
        self.set_work_time()
        self.set_rest_time()
        temp_model=move_rules2.Model_repeat24(self.args_model, self.Envir, self.home_loc, self.work_loc,
                                                              [],L_home=[],L_work=[],L_commute=[])
        temp_model.set_t_constraint(t_now=0, t_end=self.rest_time[0], args_t=self.args_t)
        temp_model.set_space_constrain(self.args_step)
        mid=temp_model.simulate4ndays(simulatenum=self.simulate_time,rest_time=[7,20],worktime=[9,18],p=p)
        return mid