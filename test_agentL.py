...
def get_point_from_geodf(path,gama):
    ...
    return pointlist
def run(path='D:\\shanghai2000\\shanghai02000grid.csv',pathod='D:\\shanghai2000\\shanghai02000od.csv'):
    args_model = [0.6, -0.21]
    args_time = [2, 1]
    args_steps = [-4.26567374722, 1.16794716154]
    gama = 1
    path=path
    pathod=pathod
    ...
    temp_path='D:\\modelE'+str(gama)+'_'+str(people_num)+"_"+getmon+getday+gethour
    ...
    #model=Model5.HomeOrWork_Model(args_model=args_model,args_t=args_time,args_steps=args_steps,environment=Envir,visited_Place=[],homeposition=random.choice(Envir.locations),workposition=random.choice(Envir.locations))
        ...
        model.simulate_home_repeat1_lognormal()
        mid=model.data_mid
        mid.person_tag = i
        write = IO.IO(mid)
